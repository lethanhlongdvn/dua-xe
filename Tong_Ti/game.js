console.log("--- SCRIPT START: Tong_Ti/game.js v3 ---");
...
(function () {
    ...
    let dbClient = null; // Renamed from supabaseClient for safety
    try {
        if (window.supabase) {
    ...
    async function updateBestTimePreview() {
        ...
        if (!best) {
            ...
            if (locals.length > 0) best = locals[0].time; // Fix: Local lưu object
        }
