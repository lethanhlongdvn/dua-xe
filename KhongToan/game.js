console.log("--- SCRIPT START: KhongToan/game.js (INFINITE MODE) ---");
...
function update() {
    if (state.mode === 'DRIVING') {
        state.targetSpeed = 15; // Chạy nhanh hơn chút cho sướng
        state.speed += (state.targetSpeed - state.speed) * 0.05;

        // Tăng quãng đường dựa trên tốc độ
...
window.startGame = function () {
    console.log("startGame called - KhongToan Mode");
    ...
    // Trong bản này, nếu muốn kết thúc thì người dùng bấm "Trang chủ"
    // Hoặc ta có thể thêm nút "Kết thúc" để lưu điểm
};

// --- HIGH SCORE (QUÃNG ĐƯỜNG) ---
const SCORE_LOCAL_KEY = 'top_scores_khong_toan';
...
async function saveHighScore(dist) {
    ...
    locals.sort((a, b) => b.distance - a.distance); // Xa nhất lên đầu
    ...
    if (dbClient) {
        ...
        await dbClient.from('high_scores').insert([{ game_id: GAME_ID, player_name: fullDisplay, score_time: dist }]); // Dùng score_time làm distance
        showLeaderboard();
    }
}
...
function init() {
    console.log("Initialization KhongToan started...");
